...
legend("topright", legend = c("Treatment","Control"), col=c("red","blue"), lty=c(1:1),lwd=c(3,3),seg.len=4)
...
## For number 4, if you wanted to connect the points you can use type="l" and the lines() function 
## inside your print.medicalRecord() function
print.medicalRecord <- function(med){
  x <- data.frame(med$date_of_admission,med$pulse,med$temperature,med$fluid_intake)
  x <- x[order(med$date_of_admission),]
  colnames(x) <- c("Date of admission","Pulse","Temperature","Fluid intake")
  rownames(x) <- NULL
  plot(x[,1],x[,2],xlab="Date",ylab="Pulse",type="l")
  lines(x[,1],x[,3],xlab="Date",ylab="Temperature")
  lines(x[,1],x[,4],xlab="Date",ylab="Fluid intake")
  return(x)
}
